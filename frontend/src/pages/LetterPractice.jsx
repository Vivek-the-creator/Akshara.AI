...
const LetterPractice = () => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      console.log(`Attempt ${newAttempts} for level ${level} - Letter: ${currentLetter.letter}`)
      ...
      if (isCorrect) {
        ...
        setMessageType('error')
        console.log(`Incorrect! Total attempts: ${newAttempts}. Keep trying!`)
  ...
  const handleNextLevel = () => {
    console.log('Current level:', level, 'Type:', typeof level)
    ...
    // Navigate to next level after a short delay to ensure modal closes
    setTimeout(() => {
      if (nextLevel <= 12) {
